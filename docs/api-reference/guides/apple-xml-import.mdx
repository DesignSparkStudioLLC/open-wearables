...
Once the upload to S3 succeeds, tell the backend so it dispatches processing. This replaces the previous S3 event notification / SNS flow.

<CodeGroup>
```bash cURL
curl -X POST "https://api.example.com/api/v1/users/{user_id}/import/apple/xml/s3/complete" \
  -H "accept: application/json" \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
  -d '{"file_key": "<file_key from Step 1>"}'
```

```python Python
complete_response = requests.post(
    f"https://api.example.com/api/v1/users/{user_id}/import/apple/xml/s3/complete",
    headers={
        "accept": "application/json",
        "Content-Type": "application/json",
        "Authorization": f"Bearer {access_token}",
...
```javascript JavaScript
await fetch(
  `https://api.example.com/api/v1/users/${userId}/import/apple/xml/s3/complete`,
  {
    method: 'POST',
    headers: {
      'accept': 'application/json',
    ...
    body: JSON.stringify({ file_key: presignedData.file_key }),
  }
);
```
</CodeGroup>
...
<Note>
  The `file_key` must be the one returned by Step 1 - it is scoped to the user (`{user_id}/raw/...`); other keys are rejected with `403`.
</Note>
  ...
  <Tab title="S3 Presigned URL (Recommended)">
    <Steps>
      ...
      <Step title="Confirm the Upload">
        ```python
        # Tell the backend the upload finished; this dispatches processing.
        complete_response = requests.post(
            f"{API_URL}/api/v1/users/{USER_ID}/import/apple/xml/s3/complete",
            headers=headers,
            json={"file_key": presigned_data["file_key"]},
        )
        complete_response.raise_for_status()
        print("✓ Processing dispatched")
        ```
      </Step>

      <Step title="Processing Happens in the Background">
        After the confirm call the backend:
        - Verifies the object exists in the bucket
        - Dispatches the `process_aws_upload` Celery task, which downloads and parses the XML
        - Imports workouts and time series data to the database
      </Step>
    </Steps>
    ...
    def main():
        ...
        # Step 4: Confirm the upload so the backend dispatches processing
        complete_response = requests.post(
            f"{API_URL}/api/v1/users/{USER_ID}/import/apple/xml/s3/complete",
            headers=headers,
            json={"file_key": presigned_data["file_key"]},
        ...
        complete_response.raise_for_status()
        print("✓ Processing dispatched - it will run in the background")

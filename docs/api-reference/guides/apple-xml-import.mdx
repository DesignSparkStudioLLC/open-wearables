...
1. **S3-compatible multipart upload** (Recommended) - For large files, uploads parts directly to AWS S3 or MinIO, reports progress, and explicitly queues processing.
...
## Method 1b: Multipart Upload (S3 or MinIO)

The portal uploads large exports using **multipart upload**, which splits the file into
parts uploaded in parallel via presigned URLs. It works identically against AWS S3 and a
self-hosted **MinIO** server, so a local deployment with no AWS account can still accept
multi-gigabyte Apple Health exports through the UI.
...
<Info>
  **Completion mode** — `APPLE_XML_UPLOAD_COMPLETION_MODE` controls how a finished upload
  starts processing: `client` (default) dispatches from the `/complete` call and works for
  S3 and MinIO; `sns` instead waits for an S3 bucket event via SNS (AWS only). Only the
  selected mode dispatches, so a file is never processed twice. Point the whole flow at
  MinIO by setting `AWS_ENDPOINT_URL` (for example, `http://minio:9000`). If the backend
  hostname is not reachable from the browser, also set `AWS_PUBLIC_ENDPOINT_URL` to the
  browser-facing origin (for example, `http://localhost:9000`).
</Info>

### Storage configuration reference

These environment variables tune the S3/MinIO XML upload flow. All are optional; the
defaults target real AWS S3.

| Variable | Default | Purpose |
| --- | --- | --- |
| `AWS_ENDPOINT_URL` | _(unset — real AWS)_ | Custom S3-compatible endpoint (e.g. `http://minio:9000`). When set, the shared client factory forces **path-style addressing + SigV4** so presigned and multipart uploads behave consistently across S3 and MinIO. Used by every backend-side S3 call. |
...
| `APPLE_XML_UPLOAD_COMPLETION_MODE` | `client` | How a finished upload triggers import: `client` dispatches from the `/complete` call (S3 and MinIO); `sns` waits for an S3 bucket event via SNS (AWS only). Only the selected mode dispatches, so a file is never processed twice. |
...
| `APPLE_XML_MULTIPART_PART_SIZE_BYTES` | `104857600` (100 MiB) | Recommended browser chunk size returned by `create` (accepts 5 MiB - 5 GiB). Clients should use the returned `part_size`, not hard-code it. |
  ...
  <Tab title="S3 Presigned POST (SNS mode)">
    <Steps>
      ...
      <Step title="Processing Happens Automatically">
        ...
        what the portal uses and which works for both S3 and MinIO.
      </Step>
    </Steps>

...
<Info>
  **Steps 2–5 apply only to `sns` completion mode.** Running the default `client` mode? Skip
  ahead to [Step 6](#step-6-update-environment-variables) - you need the bucket above, credentials,
  and a CORS policy, but no SNS topic, subscription, or S3 event notification.
</Info>

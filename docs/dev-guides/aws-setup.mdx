...
variables to run the same upload flow against a self-hosted **MinIO** (or other
S3-compatible) server, and the rest to tune upload behavior.
...
| `APPLE_XML_UPLOAD_COMPLETION_MODE` | `client` | How a finished upload triggers import: `client` dispatches from the `/complete` call (S3 and MinIO); `sns` waits for an S3 bucket event via SNS (AWS only, as configured above). Only the selected mode dispatches, so a file is never processed twice. |
...
<Note>
  Browser multipart part uploads read the `ETag` from the `PUT` response, so the bucket's
  CORS policy must allow `PUT` and **expose the `ETag` header** (`ExposeHeaders: ["ETag"]`).
</Note>

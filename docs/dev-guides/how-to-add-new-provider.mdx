...
class SuuntoData247(Base247DataTemplate):
...
- A plain `int` is accepted where the write path can't report a split (sleep
  sessions merged one at a time, say). It counts toward `result.written`, just
...
The orchestrator reads `result.written` for the headline item count,

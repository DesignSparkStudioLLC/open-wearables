...
3. Override `BaseProviderStrategy.on_connect` to enqueue that task for a newly linked connection — the OAuth callback calls it once the token is stored. For teardown, use the existing `BaseOAuthTemplate.deregister_user` rather than a new hook: disconnect, data purge and account deletion all already call it while the connection is still active.
...
- [ ] If `webhook_subscription_per_user=True`: implemented `register_user_subscriptions()` on your webhook service, overrode `on_connect` and `deregister_user`, and verified the fan-out registers one subscription per active connection

...
1. Make `register_subscriptions(callback_url)` dispatch one `sync_provider_user_subscription` task per active connection (`UserConnectionRepository.get_all_active_by_provider`) instead of registering anything itself. The `callback_url` argument is unused — each per-user task builds its own.
2. Implement `reconcile_user_subscriptions(db, user_id)` on your `BaseWebhookService`. That is what the per-user task calls; reconcile against the provider's currently configured live-sync mode (`ProviderSettingsRepository.get_live_sync_mode`), listing the provider's existing subscriptions first and changing only the gap so a redelivered task is safe.
3. Override `BaseProviderStrategy.on_disconnect` to tear a user's subscriptions down while their tokens are still valid — disconnect, data purge and account deletion all call it.
...
Because each connection holds its own subscription, `register_provider_webhooks` is dispatched on *any* live-sync mode change for these providers, not only the switch to `webhook`, so that switching back to `pull` revokes them. `app/services/providers/withings/notify_service.py` is the reference implementation.
...
- [ ] If `webhook_subscription_per_user=True`: implemented `BaseWebhookService.reconcile_user_subscriptions()`, overrode `on_disconnect`, and verified the fan-out reconciles one subscription per active connection

...
<AccordionGroup>
  <Accordion icon="docker" title="Using Docker (Recommended)">
    ...
    - 🗄️ **SeaweedFS S3 API**: http://localhost:8333
    - 📁 **SeaweedFS Filer UI**: http://localhost:8888
    
    <Tip>Use `docker compose logs -f` to view logs from all services.</Tip>

    <Tip>
      To store raw provider payloads, FIT files, or Apple Health XML uploads locally,
      follow the [local SeaweedFS deployment guide](/dev-guides/raw-payload-storage#local-deployment-with-seaweedfs).
    </Tip>
  </Accordion>
